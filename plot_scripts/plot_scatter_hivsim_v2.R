...
cis_julia <- readRDS("results/CIs/hivsim_fit_results.rds")
...
add_true_values <- cis_julia_au[c(4,5,12,16)]
...
p_omega <- ggplot(cis_julia_au, aes(x = true_omega, y = omega,
  ...
  scale_colour_manual(
    ...
    breaks = levels(cis_julia_au$simulator_pa_R0),
    labels = function(x) {
...
p_alpha <- ggplot(cis_julia_au, aes(x = true_alpha, y = alpha,
                                      shape = likelihood,
